    ...
    async (skillId: string, topicId: string, score: number) => {
      ...
      try {
        ...
        const newTopics = skill.topics.map((t) => {
          const updatedT = t.id === topicId ? { ...t, completed: isCompleted, score } : t;
          if (updatedT.completed) {
            completedCount++;
            completedTopics.push(updatedT.id);
          }
          if (updatedT.score !== undefined && updatedT.score < 60) {
            weakTopics.push(updatedT.id);
          }
          return updatedT;
        });
        ...
        // ⚡ Bolt: Eliminated 2 redundant SELECT queries by deriving intermediate states locally.
        // Parallelized the remaining 3 independent database UPDATE operations using Promise.all()
        // to prevent network waterfalls and significantly reduce overall latency.
        await Promise.all([
          supabase
            .from("user_topics")
