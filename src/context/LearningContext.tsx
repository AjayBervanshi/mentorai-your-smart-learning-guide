...
export function LearningProvider({ children, userId }: { children: React.ReactNode; userId: string | null }) {
  ...
  const updateSkillProgress = useCallback(
    async (skillId: string, topicId: string, score: number) => {
      ...
      try {
        // ⚡ Bolt: Derived new state locally to eliminate redundant network SELECT request.
        ...
        for (const t of targetSkill.topics) {
          if (t.id === topicId) {
        ...
        // ⚡ Bolt: Parallelized independent DB queries with Promise.all to avoid a network waterfall.
        const updateTopicPromise = supabase
          .from("user_topics")
        ...
        const selectLpPromise = supabase
          .from("user_learning_profiles")
