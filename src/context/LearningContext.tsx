  ...
  const updateSkillProgress = useCallback(
    async (skillId: string, topicId: string, score: number) => {
      ...
      try {
        ...
        // ⚡ Bolt: Eliminate redundant SELECT queries and network waterfalls
        // by deriving the new completed state and streaks locally from the Context,
        // and parallelizing independent database UPDATE calls using Promise.all().
        await Promise.all([
          supabase
            .from("user_topics")
        ...
        setProfile((prev) => {
          ...
          const skills = prev.skills.map((skill) => {
            ...
            const topics = skill.topics.map((t) =>
              t.id === topicId ? { ...t, completed: isCompleted, score } : t
            );
            ...
            return {
              ...skill,
              topics,
              progress: Math.round((countCompleted / topics.length) * 100),
              completedTopics,
              weakTopics,
              currentTopicIndex: Math.min(countCompleted, topics.length - 1),
            };
          });
          return {
            ...prev,
