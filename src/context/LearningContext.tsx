  ...
  const updateSkillProgress = useCallback(
    async (skillId: string, topicId: string, score: number) => {
      ...
      try {
        ...
        // ⚡ Bolt: Derived allTopics locally from the current state to eliminate
        // the redundant and blocking SELECT query.
        const allTopics = skill.topics.map((t) =>
        ...
        // ⚡ Bolt: Parallelized the independent Supabase database requests down
        // to a single network waterfall by wrapping them in Promise.all().
        const [
          topicUpdateResult,
          skillUpdateResult,
          { data: lpData }
