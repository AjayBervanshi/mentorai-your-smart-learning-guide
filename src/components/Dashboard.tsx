...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  // ⚡ Bolt: Memoized expensive derivations to prevent O(N*M) recalculations on every render
  const stats = useMemo(() => {
    if (!profile) {
      return {
        totalTopics: 0,
  ...
  const { totalTopics, completedTopics, totalProgress, currentSkill, currentTopic, weakTopicsList } = stats;
