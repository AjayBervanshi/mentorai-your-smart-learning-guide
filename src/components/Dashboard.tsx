...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  // ⚡ Bolt: Wrapped derived state aggregations in useMemo to prevent O(N*M) recalculations
  // on every keystroke when typing in the rapid local state input (newSkill).
  const { totalTopics, completedTopics, weakTopicsList, totalProgress, currentTopic } = useMemo(() => {
    if (!profile) {
      return { totalTopics: 0, completedTopics: 0, weakTopicsList: [], totalProgress: 0, currentTopic: undefined };
    }
    ...
    const topic = currentSkill?.topics[currentSkill.currentTopicIndex];
