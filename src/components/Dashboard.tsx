...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  // Memoized to prevent recalculation on every keystroke in the "Add Skill" input.
  const { totalProgress, totalTopics, completedTopics, currentSkill, weakTopicsList, currentTopic } = useMemo(() => {
    if (!profile) return { totalProgress: 0, totalTopics: 0, completedTopics: 0, currentSkill: undefined, weakTopicsList: [], currentTopic: undefined };
    let totalProgressSum = 0;
    let totalTopicsCount = 0;
    let completedTopicsCount = 0;
    let currSkill: typeof profile.skills[0] | undefined = undefined;
    ...
    for (const skill of profile.skills) {
      ...
      if (!currSkill && skill.progress < 100) {
        currSkill = skill;
      }
    ...
    if (!currSkill && profile.skills.length > 0) {
      currSkill = profile.skills[0];
    }

    const calculatedTotalProgress = profile.skills.length
      ? Math.round(totalProgressSum / profile.skills.length)
      : 0;

    const currTopic = currSkill?.topics[currSkill.currentTopicIndex];
