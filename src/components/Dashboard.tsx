                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        if (window.confirm(`Are you sure you want to remove the skill "${skill.name}"? This action cannot be undone.`)) {
                          removeSkill(skill.id).then(() => toast.success(`${skill.name} removed`)).catch(() => toast.error("Failed to remove skill"));
                        }
                      }}
                      className="text-muted-foreground hover:text-destructive transition-colors p-1 rounded opacity-0 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:ring-2"
                    >
