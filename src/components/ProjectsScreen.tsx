...
import type { ProjectTemplate } from "@/data/skillTemplates";
...
function ProjectSkillCard({ skill }: { skill: UserSkill }) {
  const { projects, unlockedCount } = useMemo(() => {
    const projs = getProjectsForSkill(skill.name);
    let count = 0;
    ...
    return { projects: projs, unlockedCount: count };
  ...
  return (
    <div className="space-y-3">
      ...
      {projects.map((project: ProjectTemplate, i: number) => {
