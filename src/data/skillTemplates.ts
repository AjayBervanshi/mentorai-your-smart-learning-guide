...
// ⚡ Bolt: Use module-level TypedArrays to eliminate array allocations in this hot path
let levenshteinMaxLen = 32;
let levenshteinPrevRow = new Int32Array(levenshteinMaxLen + 1);
let levenshteinCurrRow = new Int32Array(levenshteinMaxLen + 1);
...
function levenshtein(a: string, b: string): number {
  ...
  // Manually resize buffers if needed
  ...
  for (let i = 1; i <= m; i++) {
    ...
    // Swap row references manually instead of using array destructuring
