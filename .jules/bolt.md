...
## 2024-11-20 - Optimize autocomplete search algorithms
**Learning:** Using dynamic array allocations (like 2D arrays or new rows per iteration) inside tight loops for Levenshtein distance creates heavy GC pressure and slows down autocomplete handlers.
**Action:** Pre-allocate a single max-length array buffer outside the function, and replace chained array methods (.filter, .map) with single-pass loops that use early exits.
