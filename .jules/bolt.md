...
## 2024-05-19 - [Bulk Insert Relational Data]
**Learning:** Sequential inserts for relational data (e.g., skills and their topics) in a loop create an O(N) network waterfall.
**Action:** To optimize Supabase record creation for relational data, perform a bulk insert of parent records with `.select()` to retrieve generated IDs, then map and bulk insert child records in a single secondary operation to minimize network roundtrips.
