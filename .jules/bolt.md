...
## 2024-05-18 - [Optimizing Levenshtein memory allocation in hot paths]
**Learning:** Computing Levenshtein distance on every keystroke against hundreds of items using standard arrays causes noticeable GC pressure and frame drops.
**Action:** When optimizing algorithms executed in hot paths, reuse module-level typed arrays (e.g., `Int32Array`) instead of reallocating standard JavaScript arrays per call to eliminate memory allocations and reduce GC pressure.
