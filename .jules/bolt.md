...
## 2024-06-25 - Avoid array method chains in hot React renders
**Learning:** In complex React renders over lists of data, chained array methods like `.filter().reduce()` or `.filter().length` create temporary intermediate arrays which cause excessive garbage collection and slow down rendering. This app had them heavily in `ProgressScreen.tsx` and `ProjectsScreen.tsx`.
**Action:** Replace chained `.filter().reduce()` and similar methods with simple `for...of` loops, maintaining variables for counts/sums within the render cycle.
