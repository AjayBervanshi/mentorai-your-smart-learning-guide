...
## 2024-06-17 - Optimize network waterfalls in update operations
**Learning:** Sequential `.update()` and `.select()` queries create significant network waterfalls. Deriving intermediate states (like completed counts) from local React Context eliminates redundant `SELECT` queries entirely.
**Action:** Parallelize independent database requests by wrapping them in `Promise.all()`, and compute intermediate metrics locally when the necessary state already exists in Context.
