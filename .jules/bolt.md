...
## 2026-05-07 - Refactoring Synchronous Supabase Workflows
**Learning:** In heavily state-driven applications, chaining multiple dependent `SELECT` queries to retrieve fresh state after an `UPDATE` introduces huge latency spikes.
**Action:** Always derive intermediate states (such as gamification progress and streaks) locally from the React Context, allowing you to bypass unnecessary `SELECT` network requests and consolidate parallelizable `UPDATE` statements into a single `Promise.all` block.
