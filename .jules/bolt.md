...
## 2024-05-24 - Optimize Supabase Query Waterfalls
**Learning:** Sequential Supabase database calls in React state update handlers create significant network waterfalls. Redundant SELECT queries can often be eliminated if the context already holds the state locally.
**Action:** Always derive intermediate state from local variables when possible, and wrap independent database mutations in `Promise.all` to parallelize network operations.
