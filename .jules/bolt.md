...
## 2025-05-25 - [Optimize Levenshtein Memory Allocation]
**Learning:** Re-allocating arrays dynamically on every call of a heavily-used algorithm (like `levenshtein` distance, called per keystroke during fuzzy matching) creates significant memory overhead and Garbage Collection (GC) pauses.
**Action:** Use module-scoped `Int32Array` buffers (dynamically growing if necessary) and avoid destructuring array swaps. This dramatically speeds up string-matching without impacting correctness.
