...
## 2024-06-21 - Parallelize Supabase Queries
**Learning:** Supabase network waterfalls can be avoided by eliminating redundant `SELECT` queries (deriving intermediate state locally) and wrapping independent `.update()` requests in `Promise.all()`.
**Action:** Always derive intermediate state locally if possible, and parallelize independent database requests using `Promise.all()` to eliminate sequential blocking network waterfalls.
