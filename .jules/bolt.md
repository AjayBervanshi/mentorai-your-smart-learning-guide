...
## 2026-05-07 - Bulk Insert Optimization for Relational Data
**Learning:** Sequential inserts for parent-child relations (like skills and topics) create network waterfalls. Supabase's `.insert([...]).select()` preserves input array order, allowing reliable index-based mapping.
**Action:** Always batch parent inserts, retrieve their generated IDs, and perform a single bulk insert for all child records using array indices for mapping to avoid O(N^2) complexity and reduce network roundtrips.
