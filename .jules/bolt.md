...
## 2026-05-07 - Memoized Expensive Loop Logic
**Learning:** Found an expensive local state O(N*M) loop operation running continuously on component re-render when tracking user state inputs. Placing derived loops directly within render paths is a common anti-pattern that leads to unnoticeable degradation locally but significant slow downs as array depth/size grows over time.
**Action:** Use `useMemo` to memoize expensive derivations and explicitly manage early returns to satisfy React Hook execution order (`Rules of Hooks`).
