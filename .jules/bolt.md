...
## 2024-03-24 - Memoize Dashboard profile derived state
**Learning:** Heavy O(N) processing of derived state (like aggregating scores across all skills and topics) running directly in the render body causes noticeable lag when users are typing fast into controlled inputs (like the "Add Skill" search).
**Action:** Always wrap heavy derived state computations in useMemo when the component also contains controlled interactive inputs that cause frequent re-renders.
