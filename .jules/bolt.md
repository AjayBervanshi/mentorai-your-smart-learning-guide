...
## 2026-04-30 - Optimize fuzzy matching algorithm
**Learning:** The previous implementation of the Levenshtein distance algorithm used two arrays (`prevRow` and `currRow`) that were reallocated for every function call. When combined with chaining array map/filter/sort operations and iterating over all possible skills during string fuzzy search operations (`normalizeSkillName` and `findMatchingSkills`), this resulted in high latency and garbage collection spikes. Secondary comparisons were not skipped correctly in earlier attempts, which caused incorrect logic and skewed sort order. Also, we must compare all distance variations within the length threshold, then find the minimum, rather than skipping the secondary if the primary was merely less than the threshold.
**Action:** Used single typed array (`Int32Array(100)`) pre-allocation to replace double-array matrix operations in `levenshtein`. In `normalizeSkillName` and `findMatchingSkills`, chained array operations (`.map().filter().sort().map()`) were replaced by single-pass `for` loops with early return exits based on distance threshold.
