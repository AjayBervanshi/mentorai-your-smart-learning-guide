...
## 2024-05-30 - Memoizing Dashboard Derived State
**Learning:** When a component contains both rapid local state updates (e.g., text inputs) and expensive derived state computations (e.g., nested loops aggregating arrays), failure to memoize derivations causes O(N*M) recalculations on every keystroke, which leads to layout jank.
**Action:** Always wrap heavy data derivations in `useMemo` when they sit alongside rapid state changes.
