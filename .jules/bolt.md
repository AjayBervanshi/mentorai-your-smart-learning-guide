...
## 2026-05-07 - Supabase Relational Bulk Inserts
**Learning:** Supabase's `.insert([...]).select()` returns results in the exact same order as the input array. This means we can perform bulk inserts for relational data (like parent skills and child topics) and reliably map the generated IDs back using array indices, avoiding O(N^2) matching logic or multiple network roundtrips.
**Action:** Always batch dependent Supabase creations by inserting parent records in bulk, capturing their generated IDs via `.select()`, and then mapping child records to those IDs using the index of the parent array for the subsequent bulk insert.
