...
## 2024-04-24 - Optimizing fuzzy match loops
**Learning:** Chained array methods combined with heavy function calls (like Levenshtein distance) in tight loops create significant overhead, especially without early-exits. Using a 1D array buffer for Levenshtein and filtering based on length threshold beforehand reduces GC pressure and accelerates iterations heavily (over 3x improvement).
**Action:** Always pre-filter loops with basic length or type comparisons before invoking heavy computational operations.
