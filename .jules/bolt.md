...
## 2026-05-07 - Memoizing expensive array processing in frequent-render components
**Learning:** Performing an O(N) array processing pass on complex nested structures (like a user's skills and topics) directly inside the component body can cause significant performance degradation when the component re-renders frequently (e.g., from an uncontrolled input changing state on every keystroke).
**Action:** Use `useMemo` to memoize the computation and depend strictly on the underlying data reference (`profile.skills`), ensuring the expensive iteration is only performed when the actual data changes.
