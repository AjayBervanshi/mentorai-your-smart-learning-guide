...
## 2026-06-09 - Avoid Standard Arrays in High-Frequency String Distance Computations
**Learning:** In hot paths like fuzzy searching executed on every keystroke, allocating standard JavaScript arrays (like `Array.from` or `new Array()`) per call creates massive Garbage Collection pressure. Furthermore, array destructuring (`[a, b] = [b, a]`) implicitly creates temporary objects.
**Action:** When implementing algorithms like Levenshtein distance, use module-level `Int32Array` buffers with manual dynamic reallocation (`if (n > maxLen) { maxLen = n; arr = new Int32Array(maxLen + 1); }`), and swap variables with a standard temporary variable instead of destructuring.
