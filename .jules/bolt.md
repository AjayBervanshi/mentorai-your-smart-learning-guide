...
## 2026-05-07 - [Optimize Levenshtein Distance with Reusable Typed Arrays]
**Learning:** The Levenshtein distance algorithm used for fuzzy skill search was generating excessive garbage collection overhead by creating new arrays for every distance calculation during each keystroke. Hardcoded early returns also introduced correctness bugs.
**Action:** When optimizing algorithms executed in hot paths, reuse module-level typed arrays (e.g., `Int32Array`) to eliminate memory allocations. Avoid naive fast-paths unless mathematically sound, and replace array destructuring for swapping variables with simple temporary variables to avoid object creation.
