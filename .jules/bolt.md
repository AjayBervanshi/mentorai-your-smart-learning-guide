...
## 2026-04-30 - Autocomplete fuzzy matching optimization
**Learning:** Dynamic 2D array allocations for calculating the Levenshtein distance created massive GC pressure during tight loops of autocomplete text matching (`normalizeSkillName` and `findMatchingSkills`). Also, using chained array methods like `.map().filter().sort().map()` to process all variants over static lists generated too many throwaway objects. Skipping length checks resulted in many unnecessary comparisons. Memory guidelines clearly recommend single array buffers over 2D dynamic arrays.
**Action:** Replaced dynamic 2D array allocations with a pre-allocated single `Uint16Array` for distance calculation. Replaced chained array functions with standard `for` loops, adding early exits using lengths. Optimized `findMatchingSkills` processing from ~3.6s to ~0.75s locally over 1M iterations.
