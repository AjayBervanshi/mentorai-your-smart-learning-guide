...
## 2026-05-07 - Add AI Prompt Input Validation
**Vulnerability:** Unconstrained user inputs (`skill`, `topic`, `subtopics`) and unvalidated `contentType` were passed directly into the AI prompt generator.
**Learning:** Edge functions interfacing with AI gateways need strict input validation (allowlists and length limits) to prevent prompt injection and Denial-of-Service attacks.
**Prevention:** Always validate against strict allowlists (e.g., `contentType`) and enforce explicit length limits for strings and arrays before constructing AI prompts.
