...
## 2026-05-07 - Missing Input Validation on AI Generation Edge Function
**Vulnerability:** The `generate-content` Supabase Edge Function accepted unsanitized user input (`skill`, `topic`, `subtopics`, `contentType`) and passed it directly into system and user prompts for an external AI service. This could allow prompt injection attacks or Denial of Service (DoS) by passing excessively large strings or arrays.
**Learning:** Edge Functions that interface with AI APIs must strictly validate, type-check, and limit the length/size of all dynamic inputs, even if the function requires authentication, to prevent prompt injection and resource exhaustion.
**Prevention:** Always implement explicit allowlists for enumerable parameters (like `contentType`), enforce strict type checking (`typeof string`, `Array.isArray`), and apply reasonable length boundaries to all string and array inputs before using them in external API requests or AI prompts.
