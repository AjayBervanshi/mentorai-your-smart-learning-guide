...
## 2026-05-07 - Missing Input Validation in Supabase Edge Function
**Vulnerability:** The `generate-content` Supabase Edge Function accepted user input (`skill`, `topic`, `subtopics`, `contentType`) without proper length limits, type validation, or allowlists. This could allow prompt injection, excessive cost consumption via oversized inputs, or denial-of-service attacks against the AI gateway.
**Learning:** External API parameters (like `skill`, `topic`, `subtopics`) derived from client requests must be strictly validated before passing them to internal or external systems like AI gateways. Unvalidated inputs can lead to uncontrolled AI generation costs and prompt injection vulnerabilities.
**Prevention:** Always implement explicit input validation, including type checking (`typeof`), length limits (e.g., `length > 100`), and allowlists for predefined values (e.g., `["lesson", "quiz", "interview"].includes(contentType)`), at the edge before performing business logic. Use `unknown` instead of `any` for arrays like `subtopics` and validate each element using `.some()`.
