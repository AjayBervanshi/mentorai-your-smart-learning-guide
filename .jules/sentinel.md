...
## 2024-12-05 - AI Prompt Injection and Cost Exhaustion in Edge Functions
**Vulnerability:** Supabase Edge Functions passing unvalidated user inputs (like `skill`, `topic`, `subtopics`, and `contentType`) directly into external AI gateway requests. This allows attackers to send massive strings or unapproved control parameters, leading to prompt injection, denial-of-service via memory exhaustion, or financial cost exhaustion.
**Learning:** External API requests driven by user inputs must strictly validate and bound all input parameters before making the request.
**Prevention:** Implement explicit type checking, maximum length limits for all user-provided strings and arrays, and strict allowlists for control parameters (e.g., enum values like `contentType`) prior to invoking AI models or external APIs.
