...
## 2024-05-24 - Missing Input Bounds on AI Gateway
**Vulnerability:** The Edge Function passing parameters to the AI provider lacked strict length limits and allowlists on fields like skill and topic.
**Learning:** Without explicit bounds, an attacker can supply extremely large strings or prompt injection payloads, leading to Denial of Service (DoS) and potential AI credit exhaustion.
**Prevention:** Always implement strict schema validation, including type checking, explicit length bounds, and array size limits before passing user inputs to AI generation APIs.
