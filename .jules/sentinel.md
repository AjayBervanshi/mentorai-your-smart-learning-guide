...
## 2024-05-25 - CSS Injection via dangerouslySetInnerHTML component ID
**Vulnerability:** The exported `ChartStyle` component in Shadcn UI accepted an `id` prop that was interpolated directly into a CSS string injected via `dangerouslySetInnerHTML` (`[data-chart=${id}] { ... }`). Although the parent `ChartContainer` sanitized the `id`, `ChartStyle` did not sanitize it internally.
**Learning:** Even if a parent component sanitizes inputs, any exported component that uses `dangerouslySetInnerHTML` must independently sanitize dynamic props it uses, as it could be imported and used elsewhere with unsanitized data.
**Prevention:** Always sanitize dynamic inputs immediately before using them in `dangerouslySetInnerHTML`. In this case, `const safeId = id.replace(/[^a-zA-Z0-9-]/g, "");` was added inside `ChartStyle`.
