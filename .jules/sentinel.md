...
## 2026-05-07 - Prompt Injection and DoS via Unbounded AI Inputs
**Vulnerability:** The Supabase Edge Function `generate-content` accepted dynamic inputs (`skill`, `topic`, `subtopics`, `contentType`) directly into the AI prompt without any length limits or strict allowlist validation. This created a high risk for prompt injection and Denial of Service (DoS) by allowing an attacker to send massive payloads or unsupported content types to the AI gateway, potentially exhausting credits and disrupting service.
**Learning:** External inputs passed to AI gateways must be strictly validated. Unbounded strings and arrays are prime vectors for bypassing intended prompt structures and causing resource exhaustion.
**Prevention:** Always enforce explicit length limits for all string inputs, bound array sizes, and validate ENUM-like fields (e.g., `contentType`) against strict allowlists before constructing prompts or calling AI APIs.
