...
## 2024-05-24 - AI API Endpoint Input Validation
**Vulnerability:** The `generate-content` Supabase Edge Function lacked input length validation on user-provided strings (`skill`, `topic`, `subtopics`) and allowed uncontrolled `contentType` values.
**Learning:** When passing user inputs directly into AI gateway prompts, failure to restrict input length can lead to Denial of Service (cost exhaustion) via massive context windows. Failure to restrict control parameters like `contentType` can lead to prompt injection or backend errors.
**Prevention:** Always enforce strict allowlists for control parameters and explicit length/size limits for all user-provided strings and arrays when interacting with AI APIs.
