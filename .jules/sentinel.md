...
## 2024-11-20 - Unbounded AI Prompts in Supabase Edge Functions
**Vulnerability:** The Supabase edge function `generate-content` accepted `skill`, `topic`, `subtopics`, and `contentType` directly from user requests without enforcing length limits or validating types. An attacker could send massive payloads to cause excessive AI processing, draining Lovable AI credits (Cost Exhaustion/DoS), or inject arbitrary prompt instructions via `contentType`.
**Learning:** Any user-provided data passed into an AI prompt or control structure must have strict allowlists and explicit length limits enforced *before* processing, especially in serverless edge environments interacting with metered third-party services.
**Prevention:** Always enforce strong type checking, explicit length boundaries (e.g., maximum string length of 100 characters), array size limits, and exact string enum allowlists (e.g., `["lesson", "quiz", "interview"]`) on API inputs before interacting with external AI APIs.
