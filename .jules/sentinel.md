...
## 2024-04-16 - Prevent Prompt Injection and DoS in AI Gateway
**Vulnerability:** Supabase Edge Function `generate-content` accepts dynamic user inputs for `skill`, `topic`, and `subtopics` directly into AI prompts without length limits or allowlist validation for `contentType`, creating risk of prompt injection and denial-of-service.
**Learning:** AI Gateway integrations must treat all incoming JSON payload fields as untrusted and enforce strict constraints before processing or sending them to the LLM.
**Prevention:** Always validate against strict allowlists (e.g., `contentType`) and enforce explicit string length limits and array bounds on user inputs before integrating them into LLM prompts.
