...
## 2024-06-25 - CSS Injection via Unsanitized dynamic properties in Shadcn UI Chart Component
**Vulnerability:** The Shadcn UI `ChartContainer` component dynamically injected CSS inside a `<style dangerouslySetInnerHTML={{...}} />` block. Specifically, the `color` and `key` properties derived from `chartConfig` were directly interpolated into CSS strings without sanitization. An attacker could potentially inject malicious CSS if the chart configuration were dynamically driven by user data.
**Learning:** All properties interpolated into `dangerouslySetInnerHTML` strings must be sanitized, not just the parent `id`. Additionally, when sanitizing properties intended for CSS variables, the regex must allow underscores (`_`) to accommodate standard `snake_case` API response keys.
**Prevention:** Strictly sanitize all dynamic props before interpolating them. Using regexes like `replace(/[^a-zA-Z0-9-_]/g, "")` for keys and `replace(/[;"'{}<>]/g, "")` for values safely neutralizes potential injections without breaking functionality.
