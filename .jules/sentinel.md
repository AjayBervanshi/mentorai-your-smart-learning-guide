...
## 2024-04-29 - AI Gateway Prompt Injection & DoS Prevention
**Vulnerability:** The Edge Function dynamically generated system and user prompts for an AI gateway (Lovable AI) using unvalidated JSON parameters (`skill`, `topic`, `subtopics`, `contentType`) directly from the request body. This lacked length constraints and explicit allowlists, creating an avenue for prompt injection and potential Denial-of-Service through excessively large payloads.
**Learning:** Even when using an intermediary API gateway or wrapper for AI generation, all upstream inputs that interpolate into prompt templates or structure must be rigorously validated at the Edge Function boundary. Relying on default extraction (e.g., `const { skill, topic } = await req.json();`) without type and boundary checks is a critical security risk.
**Prevention:** Always enforce strict type-checking, establish reasonable maximum length thresholds for strings and arrays, and validate enumerable parameters against exact allowlists before they are processed or passed to external AI services.
