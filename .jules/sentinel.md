...
## 2026-05-07 - AI Prompt Injection and DoS Prevention
**Vulnerability:** Unsanitized and unbounded dynamic user inputs (`skill`, `topic`, `subtopics`, `contentType`) passed to the AI prompt generation edge function.
**Learning:** Failing to restrict dynamic inputs used in AI prompts can lead to prompt injection or denial-of-service (if arbitrarily large strings or arrays are sent).
**Prevention:** Always validate against strict allowlists (e.g., `contentType`) and enforce explicit length limits for strings and arrays before passing them to external AI APIs.
