...
## 2024-05-24 - AI Gateway Prompt Injection & DoS Prevention
**Vulnerability:** Unbounded user inputs (skill, topic, subtopics arrays) were passed directly to the Supabase Edge Function `generate-content` and then into AI prompts.
**Learning:** This presents a significant risk for prompt injection and denial-of-service (cost exhaustion) because malicious users can send massive strings or arbitrarily large arrays to run up AI credit costs.
**Prevention:** Always enforce explicit length and size limits for all user-provided strings and arrays before passing them to AI prompts. Implement strict allowlists for control parameters.
