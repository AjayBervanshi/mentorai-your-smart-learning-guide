...
## 2025-03-08 - Added Input Validation to Edge Function Prompts
**Vulnerability:** User inputs passed to AI prompt generation lacked strict allowlists and explicit length/size limits, posing a DoS (cost exhaustion) risk and prompt injection vulnerabilities. Catch blocks were leaking internal environment errors to the client.
**Learning:** External API inputs used in AI prompts or database queries must strictly validate lengths and arrays to avoid uncontrolled AI usage costs. Error messages must fail securely without leaking stack traces or internal secrets.
**Prevention:** Always enforce allowlists for control parameters and explicit constraints (e.g., slicing strings and array lengths) for all user-provided strings arrays passed to edge functions. Return generic errors to clients to avoid exposing internal structure.
