...
## 2024-07-25 - Prompt Injection and DoS via Missing Input Validation
**Vulnerability:** The `generate-content` Supabase Edge Function directly interpolated user-provided inputs (`skill`, `topic`, `subtopics`, `contentType`) into AI prompts without validating length, type, or allowed values. This could lead to prompt injection, excessive AI token usage (denial of service/financial exhaustion), and unpredictable behavior.
**Learning:** When dynamic inputs are used to construct AI prompts, they must be strictly validated. AI providers charge by the token, making unconstrained inputs a direct financial and availability risk.
**Prevention:** Always enforce strict validation rules for data destined for AI prompts: check types, apply reasonable string length limits (e.g., 100-200 characters), validate array lengths, and use allowlists for specific configurations (like `contentType`).
