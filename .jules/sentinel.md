...
## 2025-02-28 - Prompt Injection and DoS via Unvalidated AI Gateway Inputs
**Vulnerability:** The Supabase Edge Function `generate-content` accepted user input for `skill`, `topic`, `subtopics`, and `contentType` directly from `req.json()` and passed them into system and user prompts for the `ai.gateway.lovable.dev` API without any length limits or allowlists. This allowed malicious users to submit excessively large strings (causing potential DoS or exhausting API credits) and arbitrary `contentType` values (enabling prompt injection).
**Learning:** When passing dynamic user input to an AI API or gateway, missing strict allowlists and length limits exposes the application to prompt injection, denial of service, and unexpected resource consumption.
**Prevention:** Always validate dynamic inputs intended for AI prompts against strict allowlists (e.g., `["lesson", "quiz", "interview"]` for `contentType`) and enforce explicit string length limits (e.g., max 100 characters for names and topics).
