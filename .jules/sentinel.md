...
## 2025-06-01 - Input Validation Missing in Edge Function
**Vulnerability:** User inputs to the AI generation edge function (`skill`, `topic`, `subtopics`, `contentType`) were not validated for type, length, or allowed values before being used in AI prompts or database queries.
**Learning:** External user inputs passed to AI gateways or database lookups must be strictly validated (allowlists for types, string length limits) to prevent prompt injection, DoS by exhausting AI credits, and DB injection risks.
**Prevention:** Always enforce strong type checking and explicit length/size limits for all user-provided strings and arrays in Edge Functions before usage.
