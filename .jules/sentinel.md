...
## 2025-02-28 - Prompt Injection and Denial of Service Risk in AI Gateway Edge Function
**Vulnerability:** The Supabase edge function `generate-content` accepted arbitrary user input for `skill`, `topic`, `subtopics`, and `contentType` without explicit length restrictions or control parameter allowlisting. This could allow malicious users to craft excessively large payloads or unsupported control parameters, leading to prompt injection, potential prompt leakage, and denial of service (DoS) via token/cost exhaustion of the underlying Lovable AI Gateway.
**Learning:** Any user input interpolated into AI prompts or used to dictate logic pathways (like `contentType`) represents a significant attack surface. Without limits, an attacker can intentionally send massive strings to deplete AI credits quickly.
**Prevention:** Always enforce strict validation on input intended for AI prompts. Implement explicit length limits on all user-provided strings and arrays, and enforce strict allowlists for control parameters (e.g., ensuring `contentType` is only 'lesson', 'quiz', or 'interview').
