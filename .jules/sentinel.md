...
## 2025-02-28 - Missing Input Validation and Length Limits in Edge Functions
**Vulnerability:** The Supabase Edge Function `generate-content` lacked explicit input validation, type checking, and size limits for user-provided data (`skill`, `topic`, `subtopics`, `contentType`) before passing them into the AI prompt and database queries. This could lead to prompt injection, Denial of Service (cost exhaustion due to massive token usage), or cache pollution.
**Learning:** External-facing serverless functions that consume user input to generate AI prompts must enforce strict data contracts. Without explicit limits (e.g., maximum string length, max array size, allowlists for enums), malicious users can manipulate prompt generation logic or inflate API costs.
**Prevention:** Always implement strict input validation at the entry point of the function. Enforce types (e.g., `typeof x === 'string'`), define reasonable size limits (e.g., string length <= 200, array length <= 20), and use allowlists for control parameters (e.g., restricting `contentType` to known values).
