...
## 2026-05-07 - Missing Input Length Limits and Validation in AI Edge Function
**Vulnerability:** The `generate-content` Edge Function passed user-provided `skill`, `topic`, and `subtopics` directly into AI prompts without length limits or type checking, and did not strictly validate `contentType`. This could allow attackers to send massive payloads causing Denial of Service (DoS), exhausting AI credits, or performing prompt injection.
**Learning:** Even internal API endpoints must rigorously validate all inputs, especially when those inputs are forwarded to external AI providers where usage is metered or where injection is possible.
**Prevention:** Always enforce strict length limits for strings and arrays, and use allowlists for enumerations (like `contentType`) before passing user inputs to AI gateways or database queries.
