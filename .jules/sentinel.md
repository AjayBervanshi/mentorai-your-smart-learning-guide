...
## 2026-05-30 - Missing Input Validation on Control Parameters
**Vulnerability:** The `contentType` parameter in `supabase/functions/generate-content/index.ts` was used directly in database operations (`.eq("content_type", contentType)`) and state (`cacheData`) without validation against an allowlist, potentially leading to cache poisoning or DB exhaustion via arbitrary payloads.
**Learning:** Unvalidated control parameters (like content types, roles, or modes) sent directly to database queries or cache lookups represent an attack vector even if they don't immediately trigger functional errors. They can pollute the database.
**Prevention:** Always enforce strict allowlists for control parameters provided by the client (e.g., `["lesson", "quiz", "interview"].includes(contentType)`), and return a generic `400 Bad Request` securely if validation fails.
