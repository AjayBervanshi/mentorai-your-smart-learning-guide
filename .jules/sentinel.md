...
## 2024-06-28 - Unbounded User Input Passed to AI Prompt Generation
**Vulnerability:** User inputs (skill, topic, subtopics) were passed directly to AI prompt generation without length constraints or allowlists, risking prompt injection and denial-of-service (cost exhaustion).
**Learning:** Supabase Edge Functions integrating with AI APIs must enforce strict allowlists for control parameters and explicit limits for all strings/arrays to protect against malicious payloads.
**Prevention:** Implement rigorous type and length validation (e.g., max 200 chars for strings, max 20 items for arrays) for all user data before it is embedded into AI prompts.
