...
## 2025-03-01 - Missing Input Validation on AI Prompt Generation Edge Function
**Vulnerability:** The `generate-content` Supabase Edge Function processed user-supplied strings (`skill`, `topic`, `subtopics`, `contentType`) and interpolated them directly into AI prompts without enforcing length limits, type checking, or allowlists. This exposed the application to potential prompt injection and Denial of Service (DoS) attacks via oversized payloads.
**Learning:** Endpoints that dynamically construct prompts for large language models must strictly validate and sanitize all inputs. Unbounded strings can lead to excessive API costs, degraded performance, or successful prompt injection attacks.
**Prevention:** Always enforce strict allowlists for enum-like fields (e.g., `contentType`) and implement explicit length and type constraints on free-text inputs before processing them or interacting with external AI APIs.
