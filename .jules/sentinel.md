...
## 2024-07-01 - Input Validation for Edge Functions
**Vulnerability:** Edge function input parameters were passed to AI API and database queries without explicit type and length validation, posing risks of prompt injection and denial of service.
**Learning:** Even when inputs are destined for external AI services or database queries via Supabase client, missing schema validation on edge functions exposes the application to malformed or malicious payload attacks.
**Prevention:** Always implement strict schema validation, including explicit type checking, length bounds, and allowlists for enumerations, for all user input processed by edge functions before further processing.
