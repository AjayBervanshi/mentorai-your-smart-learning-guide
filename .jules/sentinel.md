...
## 2024-05-27 - Prompt Injection and DoS via Unvalidated AI Inputs
**Vulnerability:** The Supabase Edge Function `generate-content` accepted user-provided `skill`, `topic`, and `subtopics` without any length limits or type validation before embedding them into the AI prompt. This exposed the system to Prompt Injection attacks and Denial-of-Service (cost exhaustion) via massive payloads sent to the Lovable AI Gateway.
**Learning:** Edge functions that proxy user input to LLMs must strictly validate input types, enforce length constraints, and validate control parameters against an allowlist to prevent prompt manipulation and resource abuse.
**Prevention:** Always implement robust input validation (type checking, length bounds, array size limits, and allowlists for enums like `contentType`) immediately upon parsing `req.json()` in edge functions before interacting with AI APIs.
