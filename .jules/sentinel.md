...
## 2024-04-24 - AI Prompt Injection and DoS Risk
**Vulnerability:** Edge function accepted unbounded user input and unchecked `contentType` directly into AI prompts.
**Learning:** Dynamic user inputs in serverless functions that proxy to AI models must be strictly validated before processing to prevent malicious prompt injection or massive token usage (DoS).
**Prevention:** Always enforce explicit string length limits and validate structural inputs against strict allowlists (e.g., `contentType`) before invoking AI APIs.
