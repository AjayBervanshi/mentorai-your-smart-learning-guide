...
## 2026-04-30 - Open Redirect Vulnerability Mitigation
**Vulnerability:** The application was using `window.location.origin` for authentication redirects, which could be exploited in Open Redirect attacks if the window context is manipulated.
**Learning:** Relying solely on client-side origin for security-sensitive redirects like authentication can expose the application to Open Redirect vulnerabilities.
**Prevention:** Use a dedicated utility function (`getAppUrl()`) that prioritizes a trusted environment variable (`VITE_APP_URL`) over `window.location.origin`, ensuring a safe and predictable base URL for redirects.
