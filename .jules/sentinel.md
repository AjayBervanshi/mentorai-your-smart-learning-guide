...
## 2026-05-07 - Validate AI Prompt Inputs
**Vulnerability:** Input parameters fed directly to external AI providers (e.g. `skill`, `topic`, `subtopics` sent to Gemini gateway via Edge Functions) without validation exposed the application to Prompt Injection attacks, denial-of-service via cost exhaustion, and 400 Bad Request errors if length boundaries were exceeded.
**Learning:** Never pass raw user input or unvalidated JSON payloads to expensive/sensitive LLM prompts without strictly enforcing types, size limits, and enumerating acceptable values for control parameters.
**Prevention:** Use explicit type checking, length constraints, and array validation before incorporating variables into AI generation prompts.
