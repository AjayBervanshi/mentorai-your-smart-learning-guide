...
serve(async (req) => {
  ...
  try {
    ...
    // Security Fix: Enforce length limits and allowlists to prevent prompt injection and DoS
    if (typeof skill !== "string" || skill.length > 100 || typeof topic !== "string" || topic.length > 100) {
      return new Response(JSON.stringify({ error: "skill and topic must be strings under 100 characters" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    if (subtopics && (!Array.isArray(subtopics) || subtopics.length > 10 || subtopics.some((s: unknown) => typeof s !== "string" || s.length > 100))) {
      return new Response(JSON.stringify({ error: "subtopics must be an array of strings under 100 characters (max 10 items)" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
