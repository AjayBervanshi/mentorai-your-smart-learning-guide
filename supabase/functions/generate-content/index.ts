...
serve(async (req) => {
  ...
  try {
    ...
    // Security Fix: Enforce length limits and type validation to prevent DoS/prompt injection
    ...
    if (subtopics && (!Array.isArray(subtopics) || subtopics.length > 20 || subtopics.some(s => typeof s !== 'string' || s.length > 100))) {
