...
serve(async (req) => {
  ...
  try {
    ...
    // Security: Enforce explicit limits and allowlists to prevent prompt injection and DoS
    if (typeof skill !== "string" || skill.length > 100 || typeof topic !== "string" || topic.length > 100) {
      return new Response(JSON.stringify({ error: "Inputs exceed maximum length" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
