...
serve(async (req) => {
  ...
  try {
    ...
    // Security Fix: Validate input lengths and types to prevent prompt injection and cost exhaustion
    ...
    if (subtopics) {
      if (!Array.isArray(subtopics) || subtopics.length > 20 ||
          !subtopics.every(s => typeof s === 'string' && s.length <= 100)) {
