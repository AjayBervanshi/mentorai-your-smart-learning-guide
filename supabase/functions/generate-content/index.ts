...
serve(async (req) => {
  ...
  try {
    ...
    if (!skill || !topic || typeof skill !== "string" || typeof topic !== "string") {
      return new Response(JSON.stringify({ error: "skill and topic are required and must be strings" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    if (skill.length > 100 || topic.length > 100) {
      return new Response(JSON.stringify({ error: "skill and topic must be 100 characters or less" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    if (subtopics && (!Array.isArray(subtopics) || subtopics.length > 10 || subtopics.some(s => typeof s !== "string" || s.length > 100))) {
      return new Response(JSON.stringify({ error: "subtopics must be an array of strings (max 10), each 100 chars or less" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
