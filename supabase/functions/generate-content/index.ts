...
serve(async (req) => {
  ...
  try {
    ...
    if (skill.length > 200 || topic.length > 200) {
      return new Response(JSON.stringify({ error: "skill or topic length exceeds limit" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    if (subtopics && (!Array.isArray(subtopics) || subtopics.length > 20 || subtopics.some((s: unknown) => typeof s !== "string" || s.length > 200))) {
