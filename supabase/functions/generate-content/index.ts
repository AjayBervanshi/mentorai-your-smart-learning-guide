...
serve(async (req) => {
  ...
  try {
    ...
    if (!skill || typeof skill !== 'string' || skill.length > 100 || !topic || typeof topic !== 'string' || topic.length > 200) {
      return new Response(JSON.stringify({ error: "skill and topic are required and must be within length limits" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    if (subtopics && (!Array.isArray(subtopics) || subtopics.length > 10 || subtopics.some((s: unknown) => typeof s !== 'string' || s.length > 100))) {
      return new Response(JSON.stringify({ error: "subtopics must be an array of strings within length limits" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
