...
serve(async (req) => {
  ...
  try {
    ...
    // Security: Enforce explicit length limits to prevent prompt injection & DoS
    if (!skill || typeof skill !== "string" || skill.length > 100) {
      return new Response(JSON.stringify({ error: "skill is required and must be under 100 characters" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    if (!topic || typeof topic !== "string" || topic.length > 100) {
      return new Response(JSON.stringify({ error: "topic is required and must be under 100 characters" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    if (subtopics && (!Array.isArray(subtopics) || subtopics.length > 20 || subtopics.some((s: unknown) => typeof s !== "string" || s.length > 100))) {
      return new Response(JSON.stringify({ error: "subtopics must be an array of up to 20 strings (under 100 chars each)" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
